...
### Fixed
- **Executor Safety Denylist False Positives**: Resolved issue where `--format` flags (and other parameters containing substrings like `rm`) were blocked by substring matching in `CommandExecutor.is_safe_command`. Replaced naive substring matching with distinct binary check and regex word-boundary pattern matching.
- **Docker Container Inventory & NDJSON Parsing**: Implemented primary container discovery via Docker newline-delimited JSON (`docker ps -a --format '{{json .}}'`) with rich label, mount, port, network, and compose project parsing, verified against Docker Engine 29.8.1 on live Linux hosts.
